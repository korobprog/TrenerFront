...
import * as huggingfaceApi from '../../lib/utils/huggingfaceApi';

/**
 * API-обработчик для генерации текста с использованием Hugging Face API
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Запрос на генерацию текста:', {
    ...
    const result = await huggingfaceApi.getAnswer(
      text,
      session.user.id,
      category,
      company,
    ...
    if (error.message.includes('Превышен дневной лимит')) {
      ...
      statusCode = 502;
      errorMessage = 'Ошибка при обращении к Hugging Face API';
    }
