...
import * as anthropicApi from '../../lib/utils/anthropicApi';

/**
 * API-обработчик для генерации текста с использованием Claude API
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Запрос на генерацию текста с использованием Claude API:', {
    ...
    const result = await anthropicApi.getAnswer(
      text,
      session.user.id,
      category,
      company,
    ...
    console.error(
      'Ошибка при генерации текста с использованием Claude API:',
      error
    );
    ...
    if (error.message.includes('Превышен дневной лимит')) {
      ...
      statusCode = 502;
      errorMessage = 'Ошибка при обращении к Claude API';
    }
