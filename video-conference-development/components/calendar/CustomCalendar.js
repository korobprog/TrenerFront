  ...
  const loadEvents = async () => {
    try {
      ...
      const response = await fetch(
        `/api/custom/calendar-events?userId=${userId}&date=${date.toISOString()}&view=${view}`
      );
  ...
  const handleEventSave = async (eventData) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Ошибка сохранения события:', response.statusText);
      }
    } catch (error) {
  ...
  const handleEventDelete = async (eventId) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Ошибка удаления события:', response.statusText);
      }
    } catch (error) {
