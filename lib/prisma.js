...
// Логи только в режиме разработки и при первой инициализации
if (process.env.NODE_ENV === 'development' && !global.prismaModuleInitialized) {
  console.log(`[ES Module Prisma] Инициализация модуля (ID: ${moduleId})`);
  global.prismaModuleInitialized = true;
}
...
globalForPrisma.connectionSemaphore = globalForPrisma.connectionSemaphore || {
  ...
  release() {
    ...
    // Логи семафора убраны для предотвращения спама
  },
};
...
// Проверяем, существует ли уже экземпляр PrismaClient (логи только при первой инициализации)
if (process.env.NODE_ENV === 'development' && !global.prismaInstanceChecked) {
  console.log(
  ...
  global.prismaInstanceChecked = true;
}
...
if (
  ...
  !globalForPrisma.cleanupIntervalSet &&
  process.env.NODE_ENV === 'production'
) {
  const CLEANUP_INTERVAL = 5 * 60 * 1000; // 5 минут
  setInterval(cleanupConnections, CLEANUP_INTERVAL);
  console.log(
    `[ES Module Prisma] Настроена периодическая очистка соединений каждые ${
      CLEANUP_INTERVAL / 1000
    } секунд`
  );
  globalForPrisma.cleanupIntervalSet = true;
} else if (
  process.env.NODE_ENV !== 'production' &&
  !global.prismaCleanupLogged
) {
  console.log(
    '[ES Module Prisma] Периодическая очистка соединений отключена в режиме разработки'
  );
  global.prismaCleanupLogged = true;

  // Периодическая проверка и синхронизация счетчиков
  if (!globalForPrisma.syncIntervalSet) {
