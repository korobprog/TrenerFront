...
// Импортируем функции из обновленного prismaCommonJS модуля
const { default: prisma, withPrisma } = require('../prismaCommonJS');
...
async function getApiSettings(userId = null) {
  console.log('Начало получения настроек API OpenRouter', { userId });
  ...
  return await withPrisma(async (prisma) => {
    try {
      ...
      if (userId) {
        ...
        console.log(
          'Проверка условий для использования пользовательских настроек:',
          {
            ...
            hasApiKey: !!userSettings?.openRouterApiKey,
            apiType: userSettings?.apiType,
            isOpenRouter: userSettings?.apiType === 'openrouter',
            allConditionsMet: !!(
              userSettings &&
        ...
        if (
          ...
          const apiKey = decryptApiKey(userSettings.openRouterApiKey);
          console.log('API ключ успешно расшифрован:', !!apiKey);

          return {
            apiKey: apiKey,
            baseUrl:
              userSettings.openRouterBaseUrl || 'https://openrouter.ai/api/v1',
            model: userSettings.openRouterModel || 'google/gemma-3-12b-it:free',
            temperature: userSettings.openRouterTemperature || 0.7,
            maxTokensPerQuestion: userSettings.openRouterMaxTokens || 4000,
          ...
          console.log(
            'Не используются пользовательские настройки API, причина:',
            {
              ...
              useCustomApiEnabled: userSettings
                ? userSettings.useCustomApi
                : false,
              hasApiKey: userSettings ? !!userSettings.openRouterApiKey : false,
              isOpenRouterType: userSettings
                ? userSettings.apiType === 'openrouter'
                : false,
            }
          );
      ...
      console.log('Поиск глобальных настроек API OpenRouter');

      const settings = await prisma.interviewAssistantSettings.findFirst({
        where: { isActive: true, apiType: 'openrouter' },
      });

      console.log('Глобальные настройки API OpenRouter:', {
        найдены: !!settings,
      ...
      if (!settings) {
        console.error(
          'Глобальные настройки API OpenRouter не найдены или неактивны'
        );
        throw new Error('Настройки API OpenRouter не найдены или неактивны');
      }

      return {
        apiKey: decryptApiKey(settings.openRouterApiKey),
        baseUrl: settings.openRouterBaseUrl || 'https://openrouter.ai/api/v1',
        model: settings.openRouterModel || 'google/gemma-3-12b-it:free',
        temperature: settings.openRouterTemperature || 0.7,
        maxTokensPerQuestion: settings.openRouterMaxTokens || 4000,
        maxQuestionsPerDay: settings.maxQuestionsPerDay || 10,
        isActive: settings.isActive,
      };
    } catch (error) {
      console.error('Ошибка при получении настроек API OpenRouter:', error);
...
/**
 * Форматирует сообщения для отправки в OpenRouter API
...
/**
 * Получает ответ от API OpenRouter
 * @param {string} question - Вопрос пользователя
 * @param {string} userId - ID пользователя
 * @param {string} category - Категория вопроса (опционально)
 * @param {string} company - Название компании (опционально)
...
async function getAnswer(
  ...
  try {
    ...
    console.log('Данные ответа от OpenRouter API:', {
      id: data.id,
      model: data.model,
      object: data.object,
    ...
    // Примерная стоимость для модели google/gemma-3-12b-it:free
    // Это бесплатная модель, но мы все равно отслеживаем использование
    const apiCost = 0;
    ...
    console.error('Ошибка при получении ответа от API OpenRouter:', error);
    console.error('Детали ошибки:', {
      message: error.message,
      stack: error.stack,
    });
...
// Экспортируем функции с использованием CommonJS синтаксиса
module.exports = {
  getAnswer,
  getApiSettings,
  formatMessages,
